...
DAC~\cite{xia2024dac}\textsuperscript{$\ddagger$}                & ConvNeXt-B & \textbf{94.67} & \textbf{95.50} & 96.43 & 93.79 \\
\midrule
...
DINOv2-GLFA~\cite{yang2025dinov2}           & DINOv2-B   & $\sim$86\,M & 86.27 & 96.83 & 88.87 \\
\midrule
...
\textbf{Ours + VQ re-rank}\textsuperscript{\S\P\ddag} & ConvNeXt-T & $\sim$28\,M & \textbf{91.89} & 96.83 & --\textsuperscript{\dag} \\
...
verified.\\
\textsuperscript{$\triangle$}\,MCCG's DenseUAV row is our own reproduction,
not a secondhand figure: trained from the official released code
\cite{shen2024mccg} for 199 epochs with the paper's stated recipe
(lr=$0.01$, batch size 8, triplet-loss weight 0.3), evaluated under the same
protocol as every other row in this table. This exceeds the DenseUAV R@1
of 83.14 previously reported for MCCG via CEUSP's comparison table
\cite{xu2025ceusp}; we could not find any stated reproduction methodology
(epochs, learning rate, or whether official code was used) for that number,
so we report our own independently verified result instead.\\
