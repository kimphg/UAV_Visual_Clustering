...
\title{Cluster-Routed Descriptor Learning with Structured Hard-Negative Mining for UAV Cross-View Localization}
...
Within the last three years, many learning-based UAV absolute localization
methods have adopted a retrieval-and-refinement paradigm. GLVL introduced a
...
Scale-aware UAV-to-satellite localization has recently been studied by
Ye et al., who estimate absolute query scale from semantic anchors such as
small vehicles and use the recovered metric scale to adapt satellite crop size
before retrieval \cite{ye2026scaleaware}, evaluating robustness on augmented
DenseUAV and UAV-VisLoc settings. Such explicit scale recovery depends on
reference objects being visible and correctly detected, which scenes dominated
by fields, water, or vegetation may not satisfy; the present work instead
relies on the retrieval descriptor itself, trained with structured hard
negatives, to separate visually similar candidates.
...
\subsection{Vision Transformers for Image Retrieval}
...
\cite{dosovitskiy2021vit}. This property is attractive for image retrieval,
where two images may share large-scale layout while differing in viewpoint,
scale, illumination, or local appearance. Early transformer retrieval studies
showed that ViT backbones can be optimized with metric-learning objectives and
entropy regularization to produce compact global descriptors for instance-level
image retrieval \cite{el2021vitretrieval}. Compared with CNN descriptors,
transformer descriptors can represent relationships between distant structures,
which is useful for aerial scenes containing roads, field boundaries, rooftops,
and shoreline patterns.

Self-supervised transformer pretraining further strengthened the use of ViT
features for retrieval. DINO showed that self-supervised ViTs learn features
with strong semantic organization and attention maps that often localize
salient regions without explicit segmentation supervision
\cite{caron2021dino}. DINOv2 extended this direction by training robust
general-purpose visual features that transfer across image distributions and
tasks without task-specific labels \cite{oquab2023dinov2}. In visual place
recognition, AnyLoc demonstrated that foundation-model features, particularly
DINO and DINOv2 descriptors aggregated with VLAD, can generalize across
viewpoints, domains, and environments without dataset-specific fine-tuning
\cite{keetha2023anyloc}. These results motivate using a strong pretrained
transformer backbone as a stable feature extractor, while adapting lightweight
heads for the target UAV localization objective.

Transformer models have also been applied directly to cross-view
geo-localization. EgoTR introduced an evolving transformer formulation to model
global dependencies between street-view and aerial imagery
\cite{zhu2021egotr}. TransGeo later argued that a transformer-only
architecture can replace several handcrafted cross-view alignment operations
by using attention to emphasize discriminative regions and suppress invalid
patches \cite{zhu2022transgeo}. These methods illustrate why transformer
attention is relevant to cross-view retrieval: the model can compare image
regions through global context rather than relying only on local convolutional
patterns.
...
The system is trained with a cross-view drone-to-satellite (D2S) supervision
signal. Two complementary datasets are used.

\paragraph{UAV-VisLoc with Game4Loc-style pairing.}
The primary training source is the UAV-VisLoc dataset \cite{xu2024uavvisloc},
processed using the Game4Loc tiling protocol \cite{ji2024game4loc}.
A pre-tiled satellite pyramid at four zoom levels serves as the gallery.
For each drone image, positive tiles are identified by geographic overlap:
a tile qualifies as a strong positive when its intersection-over-union (IoU)
with the drone footprint is $\geq 0.39$, and as a semi-positive when IoU
$\geq 0.14$. Semi-positives enter training with their IoU value as a
continuous weight for the weighted InfoNCE loss, providing a softer supervision
signal for partially overlapping views.
Training pairs are stored in a Game4Loc-format JSON file and loaded
with the \texttt{pos\_semipos} mode. The footprint for each drone image is
computed from altitude and heading metadata in the accompanying CSV files,
using a camera model with $\text{FOV}_H = 36^\circ$ and
$\text{FOV}_V = 52^\circ$.
This setup provides real D2S training signal
over diverse terrain with authentic sensor appearance differences.

\paragraph{University-1652.}
The University-1652 dataset \cite{zheng2020university1652} provides a
controlled building geo-localization benchmark with 701 training locations
across a university campus. Each location is represented by approximately
54 drone images taken from multiple viewpoints and altitudes, paired with a
single overhead satellite image. All drone images from the same location share
the satellite image as their positive, yielding approximately 37\,800
training pairs. University-1652 complements VisLoc training with diverse
viewpoints, multi-building urban scenes, and a fixed overhead satellite
perspective, improving robustness to altitude variation.

\paragraph{Training transform.}
All training images are resized to the backbone input size
($224\times224$ for Swin-T, $384\times384$ for Swin-B).
...
\subsection{Metric}
Evaluation follows the Game4Loc IoU-based matching protocol
\cite{ji2024game4loc}.
A retrieved tile $m_j$ is counted as a hit for query $q_i$ if and only if it
is a positive tile under the criterion defined in Section~\ref{sec:eval}:
\begin{equation}
  \mathrm{hit}(q_i, m_j) =
  \begin{cases}
...
The primary evaluation dataset is UAV-VisLoc \cite{xu2024uavvisloc}.
Evaluation applies the Game4Loc tiling and matching protocol directly to the
UAV-VisLoc satellite maps and drone sequences: the pre-tiled gallery
($256\times256$ px, four zoom levels per area) is embedded and clustered,
query images are matched against cluster candidates, and hits are identified
by the Game4Loc IoU criterion (IoU $\geq 0.39$ or tile recall $\geq 0.80$).
Altitude and heading metadata are read from the per-sequence CSV file to
compute drone footprints for IoU evaluation.
Table~\ref{tab:config} summarizes the evaluation configuration.

UAV-VisLoc is chosen as the primary evaluation benchmark because it provides
large georeferenced satellite maps, realistic UAV imagery with pose metadata
(altitude, heading), diverse terrains, and multiple areas — matching the
intended operating condition of the proposed pipeline. Applying the Game4Loc
protocol to UAV-VisLoc aligns hit labelling, gallery structure, and reported
metrics with the Game4Loc standard while evaluating on a real-world dataset
rather than a synthetic game engine environment.
...
Table~\ref{tab:ablation} evaluates the contribution of each architectural and
evaluation component using representative sequence-04 runs. In particular, it
tests whether the learned descriptor and cluster routing improve over flat
descriptor search.

\begin{table}[htbp]
\caption{Ablation Study. UAV-VisLoc seq.~04 (top) and GTA-UAV same-area under Game4Loc tiling (bottom). Bold = best per section per metric. $\dagger$~SDM@3 (Game4Loc paper reports SDM@3; ours are SDM@1).}
\label{tab:ablation}
\centering
\footnotesize
\setlength{\tabcolsep}{3pt}
...
\midrule
\multicolumn{6}{l}{\textit{UAV-VisLoc, seq.~04}} \\
\midrule
Flat raw Swin-T & 0.3916 & 0.6016 & -- & -- & 859.3 \\
Flat learned descriptor, cosine & 0.6233 & 0.8171 & 0.3011 & -- & 313.9 \\
Flat learned descriptor, centered & 0.6165 & 0.8225 & 0.3290 & -- & 363.0 \\
Two-stage, centered & 0.6369 & 0.9675 & 0.5289 & -- & 515.0 \\
Two-stage, cluster-Mahalanobis & \textbf{0.9187} & \textbf{1.0000} & \textbf{0.7188} & \textbf{0.6895} & 499.5 \\
\midrule
...
\caption{Comparison with state-of-the-art on SUES-200 (Recall@1, \%, drone$\to$satellite).
         SOTA values as reported in~\cite{li2025cvd}.
...
\midrule
\textbf{Ours}\textsuperscript{\S} & Swin-B & \textbf{96.30} & \textbf{98.25} & \textbf{99.00} & 99.15 \\
...
\midrule
\textbf{Ours} & Swin-B & \textbf{94.52} & \textbf{95.44} & \textbf{96.86} & \textbf{93.83} \\
...
MCCG~\cite{shen2024mccg}                     & ConvNeXt-T & $\sim$28\,M & 83.14 & 93.39 & 85.94 \\
CAMP~\cite{wu2024camp}\textsuperscript{$\diamond$}                        & ConvNeXt-B & 91.4\,M     & 88.72 & --    & --    \\
CEUSP~\cite{xu2025ceusp}\textsuperscript{$\star$}         & ConvNeXt-T & $\sim$28\,M & 89.45 & 96.05 & \textbf{91.01} \\
\midrule
...
\textbf{Ours}\textsuperscript{\S\ddag}                & ConvNeXt-T & $\sim$28\,M & 82.41 & 94.59 & 85.83 \\
\textbf{Ours + VQ re-rank}\textsuperscript{\S\P\ddag} & ConvNeXt-T & $\sim$28\,M & \textbf{91.89} & \textbf{96.83} & --\textsuperscript{\dag} \\
\textbf{Ours}\textsuperscript{\S\#}                & ConvNeXt-B & $\sim$89\,M & 84.56 & 95.02 & 87.45 \\
\textbf{Ours + VQ re-rank}\textsuperscript{\S\P\#} & ConvNeXt-B & $\sim$89\,M & 91.29 & 96.48 & --\textsuperscript{\dag} \\
\bottomrule
\end{tabular}}

\vspace{2pt}
{\footnotesize
\textsuperscript{\ddag}\,Selected ConvNeXt-T checkpoint (epoch 25 of
fine-tuning from a 20-epoch config-sweep winner: lr=$3\times10^{-4}$, last 3
backbone stages unfrozen); later checkpoints were evaluated and regressed,
so this checkpoint is the final reported model.\\
...
so this trajectory is not directly comparable to the table's row, which
re-evaluates the selected epoch-25 checkpoint under the same official
protocol as the Swin-B and ConvNeXt-T rows -- the same epoch-25-peak pattern
observed for ConvNeXt-T. Also reports R@10\,$=97.85$, mAP\,$=73.39$ (coarse)
and R@10\,$=97.85$, mAP\,$=77.71$ (VQ re-rank).}
\end{table}
